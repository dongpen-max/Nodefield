...
当前验证结果：30 个领域测试和 15 个 Playwright E2E 测试通过，TypeScript 严格检查与 Vite/PWA 生产构建通过。浏览器测试覆盖多画布隔离与持久化、V0.1 存储迁移、工作区备份恢复、损坏归档保护、单写者接管、Web Locks 兼容性阻断、离线刷新与编辑、边属性、批量操作、撤销重做和降级存储提示，并从 `390x844`、`768x1024`、`1280x800` 和 `1440x900` 独立启动验收。

## 架构

```text
src/
  components/   画布卡片、画布管理、工具坞、顶部栏、存储状态、检查器和反馈
  data/         可恢复的示例画布
